<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch">
      <el-form-item label="合作商名" prop="partnerName">
        <el-input
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['manage:partner:add']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="success" plain icon="Edit" :disabled="single" @click="handleUpdate" v-hasPermi="['manage:partner:edit']">修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" plain icon="Delete" :disabled="multiple" @click="handleDelete" v-hasPermi="['manage:partner:remove']">删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button plain icon="Download" @click="handleExport" v-hasPermi="['manage:partner:export']">导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="partnerList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" width="250" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          ...
          <el-button link type="primary" icon="Key" @click="handleResetPwd(scope.row)" v-hasPermi="['manage:partner:edit']">重置密码</el-button>
    ...
    <el-dialog :title="title" v-model="open" width="600px" append-to-body>
      <el-form ref="partnerRef" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item label="分成比例(%)" prop="profitRatio">
          <el-input-number v-model="form.profitRatio" :min="0" :max="100" />
        </el-form-item>
...
const data = reactive({
  ...
  rules: {
    partnerName: [{ required: true, message: "合作商名称不能为空", trigger: "blur" }],
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除合作商编号为"' + partnerIds + '"的数据项？').then(function() {
...
function handleResetPwd(row) {
  proxy.$modal.confirm('是否确认重置"' + row.partnerName + '"的密码为123456？').then(function() {
